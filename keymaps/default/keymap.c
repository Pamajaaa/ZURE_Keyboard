...
// スティック長押しでのモード切替ロジックはVialコンボに移行したため削除
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        if (keycode == 0x7E00) { // Mode: ANALOG
            set_stick_mode(MODE_ANALOG);
            return false;
        } else if (keycode == 0x7E01) { // Mode: TYPING
            set_stick_mode(MODE_TYPING);
            return false;
        } else if (keycode == 0x7E02) { // Mode: WASD
            set_stick_mode(MODE_WASD);
            return false;
